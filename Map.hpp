...
/*   Updated: 2020/09/28 23:33:53 by plamtenz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft {
	...
	class map
	{
		...
		bool								rbt_is_right_child(Node *target) { return (target == target->parent_node->children_right); }
		/* Returns true if "target" is the left child */
		bool								rbt_is_left_child(Node *target) { return (target == target->parent_node->children_left); }
		/* Returns a pointed reference (void *&ptr ~= void **) to the left child of "l" */
		static Node							*&btr_node_left_children(Node *l) { return (l->children_left); }
		/* Returns a pointed reference (void *&ptr ~= void **) to the right child of "r" */
		static Node							*&btr_node_right_children(Node *r) { return (r->children_right); }
		...
		void								rbt_transplant(Node *dest, Node *src) {
			if (dest->parent_node == nil)
				root = src;
			else if (rbt_is_left_child(dest))
				lhs->parent_node->children_left = src;
			else
				lhs->parent_node->children_right = src;
			src->parent_node = dest->parent_node;
		}
		...
		Node								*rbt_find_node(const key_type &key) const {
			...
			while (result != nil)
			{
				if (comp(key, result->key))
					result = result->children_left;
				else if (comp(result->key, key))
					result = result->children_right;
				else
					return (result);
			}
			return result;
		...
		Node								*bst_add_recursive(key_type &&key, mapped_type &&data, Node *parent, Node **new_node) {
			if (*new_node == nil)
			{
				*new_node = create_node(); // to do
				(*new_node)->parent_node = parent;
				size++;
				return (*new_node);
			}
			else if (comp(key, (*new_node)->key))
				return (rbt_add_recursive(key, data, new_node, (*new_node)->children_left));
			else if (comp((*new_node)->key, key))
				return (rbt_add_recursive(key, data, new_node, (*new_node)->children_right));
			else
				return (NULL);
		}
		/* Add a new node allocating it in the tail of one of the branches. It uses the key given as parameter for sellect the destination branch */
		Node								*bst_add(key_type  &&key, mapped_type &&data) {
			...
			while (aux != nil)
			{
				parent = aux;
				if (comp(aux->key, key))
					aux = aux->children_right;
				else if (comp(key, aux->key))
					aux = aux->children_left;
				else
					return (NULL);
			}
		...
		void								rbt_fixed_add(Node *new_node) { 
			while (new_node->parent_node->color & RED)
			{
				if(rbt_is_left_child(new_node->parent_node))
					rbt_generic_fixed_add(&new_node, btr_node_left_children , btr_node_right_children);
				else
					rbt_generic_fixed_add(&new_node, btr_node_right_children, btr_node_left_children);
			}
			root->color &= BLACK;
		}
		...
		void							rbt_generic_fixed_delete(Node **target_node, Child1 ch1, Child2 ch2) {
			...
			if (aux->children_left->color == BLACK && aux->children_right->color & BLACK)
			{
				aux->color |= RED;
		...
		void							rbt_fixed_remove(Node *target_node) {
			while (target_node != root && target_node->color == BLACK)
			{
				if (rbt_is_left_child(target_node))
					rbt_generic_fixed_delete(&target_node, btr_node_left_children, btr_node_right_children);
				else
					rbt_generic_fixed_delete(&target_node, btr_node_right_children, btr_node_left_children);
			}
		...
		void								rbt_delete_node(Node *target_node) {
			Node *aux = target_node->children_right;
			Node *xua = target_node;
			bool color_remember = target_node->color;

			if (target_node->children_left == nil)
				rbt_transplant(target_node, aux);
			else if (target_node->children_right == nil)
				rbt_transplant(target_node, aux = aux->children_left);
			else
			{
				xua = rtb_minimum(target_node->children_right);
				color_remember = xua->color;
				aux = xua->children_right;
				if (xua->parent_node == target_node)
					aux->parent_node = xua;
				else
				{
					rbt_transplant(xua, xua->children_right);
					xua->children_right = target_node->children_right;
					xua->children_right->parent_node = xua;
				}
				rbt_transplant(target_node, xua);
				xua->children_left = target_node->children_left;
				xua->children_left->parent_node = aux;
